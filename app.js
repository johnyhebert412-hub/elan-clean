...
function renderToday() {
  $("#greeting").textContent = greetingForLocalTime();
...
function renderWorkCoach() {
  ...
  const shiftClosed = isAfter || plan.checkedIn;
  $(".work-mode-actions").classList.toggle("hidden", shiftClosed);
  $("#work-help-button").classList.toggle("hidden", shiftClosed);
  $("#work-help-panel").classList.toggle("hidden", shiftClosed);
  $("#work-checkin-button").classList.toggle("hidden", plan.checkedIn);
}
...
function showProposedMission() {
  ...
  $("#mission-card").classList.remove("timer-running");
...
function runTimer() {
  ...
  $("#direct-action-actions").classList.add("hidden");
  $("#mission-card").classList.add("timer-running");
...
function renderDomains() {
  $("#domain-list").innerHTML = Object.entries(dayTypes).map(([key, dayType]) => `
    <article class="domain-card">
      <div>
        <h3>${dayType.icon} ${dayType.label}</h3>
        <p class="muted">${dayType.focuses.map(focus => focus.label).join(" · ")}</p>
      </div>
      <button data-day-type="${key}">Choisir</button>
    </article>
  `).join("");
}
...
async function renderVersion() {
  try {
    ...
    const changes = (release.changes || []).slice(0, 3);
    $("#app-changes").replaceChildren(...changes.map(change => {
      const item = document.createElement("li");
      item.textContent = change;
      return item;
    }));
  } catch {
    // The visible fallback remains available if version data cannot be read.
  }
}
