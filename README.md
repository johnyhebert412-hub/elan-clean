...
- Publier le projet source ou le ZIP final, jamais une page locale nommee `Elan-test-local.html`.

Le build verifie automatiquement que `index.html` charge bien `app.js` comme module et bloque la publication si du JavaScript brut est place dans la page.
...
La version visible dans Parametres vient de `version.json`.

- Correction importante: `npm run release:patch -- "Resume court"`
- Nouvelle fonctionnalite: `npm run release:minor -- "Resume court"`
- Grande etape: `npm run release:major -- "Resume court"`
- ZIP final GitHub/Vercel: `npm run zip`

La commande de version met aussi a jour la date, `CHANGELOG.md` et le cache de l'application installee.
